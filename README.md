...
> Prisma** no back, **Postgres self-hosted**. Dados reais, sem mocks em runtime.

## Como rodar (full-stack local)

Pré-requisitos: Node 20+ e um Postgres acessível (local, Docker, ou o mesmo
usado em produção) e um bucket S3-compatível (MinIO) pra upload de imagens.

```bash
# 1. Configure o .env (raiz do repo)
cp .env.example .env
# Cole no .env: DATABASE_URL/DIRECT_URL (seu Postgres) e
# MINIO_ENDPOINT/MINIO_ACCESS_KEY/MINIO_SECRET_KEY/MINIO_BUCKET (seu MinIO) —
# veja o .env.example para o passo a passo completo.
...
## Deploy (servidor próprio via Coolify)

Front, back e banco rodam no mesmo servidor, como aplicações separadas
gerenciadas pelo [Coolify](https://coolify.io) (Traefik + Let's Encrypt
automático). Dois domínios (ou subdomínios), um por app — mantém o CORS
simples em vez de mesma-origem por path-routing.

**1. Banco (Postgres)** — provisione um recurso "Postgres" no Coolify (não um
docker-compose à mão — ganha backup/volume gerenciado igual aos outros bancos
do servidor). Sem pooler na frente numa instância única, então
`DATABASE_URL`/`DIRECT_URL` apontam pra mesma connection string.

**2. Storage (MinIO)** — bucket dedicado (`hub-uploads`) num MinIO
S3-compatible (pode ser uma instância própria ou compartilhada com outra
aplicação do servidor, desde que com access key **dedicada**, não
reaproveitada). Preencha `MINIO_*` no `.env.example`.

**3. App do frontend no Coolify** — Build pack = Dockerfile, **contexto de
...
processo Node persistente — não serverless). Env vars: todas as do `.env.example` (`DATABASE_URL`,
`DIRECT_URL`, `MINIO_*`, `JWT_SECRET`, `CORS_ORIGINS` apontando pro domínio do
front, etc.), cadastradas no painel do Coolify. Depois do primeiro deploy,
...
> A reconciliação de PIX roda como um `setInterval` de processo contínuo
> (`src/jobs/paymentReconciliation.ts`, iniciado por `src/server.ts`) — não
> depende de cron externo, já que o backend fica sempre no ar (processo
...
- **Postgres self-hosted**, seed idempotente
- **JWT + roles** (Client/Partner/Admin) + middlewares de autorização
- Upload de imagens direto no **MinIO** (S3-compatível)
...
backend/
  ...
  src/infra/             prisma client, auth (JWT/bcrypt), gateways de
                         pagamento, storage MinIO, settings provider
